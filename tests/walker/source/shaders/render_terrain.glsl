...
{ 
	...
	light += 0.02 * Indirect * occlusion * daylight * sunColor * max(0.0, dot(N,I)); // max(0.0, 0.2 + 0.8*dot(N,I)); 
	light += 0.03 * Sky      * occlusion *			  zenithColor * /*N.z;*/  clamp(0.2 + 0.8*N.z, 0.0, 1.0);
	light += 0.03 * Fresnel  * relief	 * (fresnelColor - light) * fresnel;
}
